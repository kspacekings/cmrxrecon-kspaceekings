...
def complex_2_numpy(x):
    ...
    """
    out = x.numpy() # N x 2 x H x W 

    #Force Channel dim to the last dim
    dims = list(range(out.ndim))
    out = np.transpose(out, (dims[0], *dims[2:], dims[1])) # N x H x W x 2
...
def numpy_2_complex(x):
    ...
    out = np.concatenate((real,imag),axis=-1) # H x W x 2

    #Force Channel dim to the first dim
    dims = list(range(out.ndim))
    out = np.transpose(out, (dims[-1], *dims[:-1])) # 2 x H x W 
